...
const Countries = ({countries, filter, setFilter}) => {
    ...
    if(filter[0] && filter[0] === 'matched'){
      const filtered = countries.find(c =>
        c.name.common === filter[1]
      )
    ...
    return(
      <table>
        <tbody>
          {filtered.map((country, index) => <ShowCountry key = {country.name.common} country = {country} filter = {filter} setFilter={setFilter} index = {index + 1}/>)}
        </tbody>
      </table>
    )
  }
